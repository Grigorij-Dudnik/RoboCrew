...
class SemanticMapOverlay:
    ...
    def apply(self, image: np.ndarray, metadata) -> None:
        try:
            ...
            if key != self._key:
                self._key = key
                self._overlay = (
                    self._render(path, metadata) if modified else None
                )
    ...
    def _render(self, path: Path, metadata) -> np.ndarray:
        ...
        for index, place in enumerate(places):
            points = np.asarray(
                [
                    self._world_to_image(x, y, metadata)
                    for x, y in place["points"]
                ],
                dtype=np.int32,
            )
            if not len(points):
    ...
    def _draw_label(overlay, text, anchor, occupied, color) -> None:
        ...
        for dx, dy in offsets:
            left = min(
                max(x + dx, 5),
                max(5, overlay.shape[1] - box_width - 5),
            )
            top = min(
                max(y + dy, 5),
                max(5, overlay.shape[0] - box_height - 5),
            )
        ...
        cv2.putText(
            overlay, text, origin, font, 0.55, (0, 0, 0, 255), 2, cv2.LINE_AA
        )
        cv2.putText(
            overlay, text, origin, font, 0.55, (*color, 255), 1, cv2.LINE_AA
        )
